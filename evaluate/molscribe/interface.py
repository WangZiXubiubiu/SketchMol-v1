...
class MolScribe:
    ...
    def predict_imagespredict_images_from_csv_helper(self, input_images: List, batch_size=16):
        device = self.device
        predictions = []

        for idx in tqdm(range(0, len(input_images), batch_size)):
            batch_images = input_images[idx:idx+batch_size]
            images = [self.transform(image=image, keypoints=[])['image'] for image in batch_images]
            images = torch.stack(images, dim=0).to(device)
            with torch.no_grad():
        ...
        node_symbols = [pred['chartok_coords']['symbols'] for pred in predictions]
        edges = [pred['edges'] for pred in predictions]
        token_scores = [np.mean(pred["chartok_coords"]["scores"]) for pred in predictions]

        smiles, molblock, r_success = convert_graph_to_smiles(node_coords, node_symbols, edges, images=input_images)
        return smiles, molblock, token_scores

    def predict_images_from_csv(self, input_csv_path: List, batch_size=16):
        input_images = []
        for path in input_csv_path:
            image = cv2.imread(path)
            image = cv2.cvtColor(image, cv2.COLOR_BGR2RGB)
            input_images.append(image)
        return self.predict_imagespredict_images_from_csv_helper(input_images, batch_size)
