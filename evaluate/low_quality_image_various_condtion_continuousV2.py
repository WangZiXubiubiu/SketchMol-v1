...
def main():
    ...
    with tqdm(total=len(all)) as pbar:
        for index, row in all.iterrows():
            ...
            # if cur_mol_property_result["SA"] > 5:
            #     target_path_csv.append([row["image_path"],
            #                             row["SMILES"],
            #                             cond_dict["None_property"],
            #                             cond_dict["None_logp"],
    ...
    # print("all imqualified image rate:", len(target_path_csv) / len(all) * 100, "%")
    ...
    print("umatch_property_images:",
          (len(target_path_csv) - error_invalid_mol) / len(all) * 100, "%")
    # for i in range(len(all_setting_each_property)):
    #     if all_setting_each_property[i] == 0:
    #         all_setting_each_property[i] = 1
    # print("match_each_property:", violate_list_each_property / all_setting_each_property)
    print("success_rate:", ( len(valid_row_index) / (len(all)-error_invalid_mol)) * 100, "%")
    ...
    pd.DataFrame(target_path_csv, columns=["image_path", "SMILES", "property_match", "aLogP_label",
                                               ...
                                               "rotatable_label_None"]).to_csv(os.path.join(save_to_dir, "low_quality_invalid_path.csv"), index=False)
    print("save to ", os.path.join(save_to_dir, "low_quality_invalid_path.csv"))
